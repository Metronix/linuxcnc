...
class Avc_process(QObject):
    def __init__(self,widgets):
        super(Avc_process,self).__init__()
        self.childFrame = widgets.findChild(QFrame, "avc_frame")
        self.process = QProcess(self)  
        self.process.readyReadStandardOutput.connect(self.on_ready_read)
        self.process.start("python3", ["/home/cnc/linuxcnc/configs/TrialBot/qtvcp/screens/qtplasmac_4x3/avc_1.py"])
    def on_ready_read(self):
        """
        Método chamado quando há dados na saída padrão do processo filho.
        Espera receber o window ID, que será utilizado para incorporar a janela.
        """
        # Lê a saída do processo filho
        data = self.process.readAllStandardOutput().data().decode().strip()
        print("Dados recebidos do filho:", data)
        try:
            win_id = int(data)
        except Exception as e:
            print("Erro convertendo o win_id:", e)
            return

        # Cria um QWindow a partir do win_id recebido
        external_window = QWindow.fromWinId(win_id)
        # Cria um container QWidget para encapsular a janela externa,
        # definindo o QFrame (self.childFrame) como pai.
        container = QWidget.createWindowContainer(external_window, self.childFrame)
        container.setMinimumSize(334, 198)  # ajuste se necessário
        

        # Se o QFrame ainda não tiver um layout, cria um e adiciona o container
        layout = self.childFrame.layout()
        if layout is None:
            layout = QVBoxLayout(self.childFrame)
            layout.setContentsMargins(0, 0, 0, 0)
            self.childFrame.setLayout(layout)
...
# called by qtvcp.py
    def initialized__(self):
        ...
        self.avc_process = Avc_process(self.w)
...
#        self.extKerfCrossEnablePin = self.h.newpin('ext_kerfcross_enable', hal.HAL_BIT, hal.HAL_IN)
    ...
    def link_hal_pins(self):
        ...
        #i2c_Read
        #CALL(['halcmd', 'net', 'plasmac:arc-voltage-set', 'qtplasmac.arc_voltage_set-f', 'i2c-rasp.arc-voltage-in'])
        #CALL(['halcmd', 'net', 'plasmac:dead-zone-arc-set', 'i2c-rasp.dead-zone', 'qtplasmac.dead_zone_arc_set-f'])
        #CALL(['halcmd', 'net', 'plasmac:max-voltage-set', 'i2c-rasp.max-volts', 'qtplasmac.max_voltage_set-f'])
        #CALL(['halcmd', 'net', 'plasmac:voltage-out', 'i2c-rasp.voltage-out', 'qtplasmac.arc_voltage_read'])
        #CALL(['halcmd', 'net', 'plasmac:dead_zone_arc', 'i2c-rasp.dead-zone-out', 'qtplasmac.dead_zone_arc'])
        #CALL(['halcmd', 'net', 'plasmac:max-voltage-out', 'i2c-rasp.max-voltage-out', 'qtplasmac.max_arc_voltage'])
        #CALL(['halcmd', 'net', 'plasmac:arc-voltage', 'i2c-rasp.voltage', 'qtplasmac.arc_voltage'])
...
#########################################################################################################################
    ...
    def set_run_button_state(self):#not self.offsetsActivePin.get() and removido para testes com o botão run com offset no z Pedro na linha 1493 antes da \
        if STATUS.machine_is_on() and STATUS.is_all_homed() and \
           STATUS.is_interp_idle() and self.plasmacStatePin.get() == 0:
...
#########################################################################################################################
    ...
    def abort_pressed(self):
        ...
        if self.manualCut:
            ...
            STATUS.emit('update-machine-log', log, 'TIME')
        ACTION.SET_MACHINE_STATE(STATUS.machine_is_on())
...
#########################################################################################################################
    ...
    def set_signal_connections(self):
        ...
        self.w.conv_traj.pressed.connect(lambda: self.conv_call('traj'))
    ...
    def set_mode(self):
        ...
        block2 = ['arc_voltage_scale', 'arc_voltage_scale_lbl', 'arc_voltage_offset', 'arc_voltage_offset_lbl',
                  ...
                  'arc_override_frm', 'voidlock_slope', 'voidlock_slope_lbl', 'thc_auto', 'thc_auto_lbl', 'arc_voltage']
        if self.mode == 1:
...
#########################################################################################################################
    ...
    def get_thc_enable(self):
        return self.w.thc_enable.get()
...
#########################################################################################################################
    ...
    def set_color_styles(self):
        ...
        buttons = ['jog_x_minus', 'jog_x_plus', 'jog_y_minus', 'jog_y_plus',
                   ...
                   'conv_line', 'conv_circle', 'conv_circle', 'conv_ellipse', 'conv_triangle',
        ...
        conv_images = ['conv_line_point', 'conv_line_angle', 'conv_line_3p',
                       'conv_line_2pr', 'conv_arc_angle', 'conv_traj_l' ,'conv_bolt_l',
